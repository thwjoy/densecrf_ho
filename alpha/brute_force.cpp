...
double compute_alpha_divergence(const MatrixXf & unaries,const std::vector<MatrixXf*> & pairwise, const std::vector<float> &pairwise_weight,const MatrixXf & approximation, double alpha){
    ...
    while (not all_conf_done) {
        ...
        D_alpha += (1/(float)(alpha * (1-alpha))) * (pow(conf_proba, alpha) * pow(approx_proba, 1-alpha) +
                                                     alpha* conf_proba +
...
double compute_probability(const VectorXi& labeling, const MatrixXf & unaries, const std::vector<MatrixXf*> pairwise, const std::vector<float> pairwise_weights, double Z){
     ...
     * pairwise is a list of pointers to matrix nb_feat by N_ containing features of pairwise filters
